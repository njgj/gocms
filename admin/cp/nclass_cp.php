...
<thead>
  ...
  <?php
	...
	case 'act_add':
		...
		$sql = "INSERT INTO g_nclass_cp (classname,parentid,orderid) VALUES('".$_POST['classname']."',".$parentid.",".$_POST['orderid'].")";
		$db -> query($sql);
...
function delclass($classid){
    ...
    if($classid){
		...
		$db->query("delete from g_nclass_cp where locate(',$classid,',classpath)>0");
		return true;
	}else{
		return false;
	}
...
function savepath($newid,$parentid){
	...
	if($parentid==0){ 
		...
		$sql='select classpath from g_nclass_cp where classid='.(int)$parentid;
		$query = $db -> query($sql);
		if($row = $db -> fetch_array($query)){
			$classpath=$row['classpath'].$newid.','; //父节点的路径	
		}
	}
	return $db -> query("update g_nclass_cp set classpath='$classpath' where classid=".(int)$newid);
}
?>
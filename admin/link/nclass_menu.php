...
<body>
<br />
<p align="center"><img src="../images/tb.gif" width="16" height="16" />&nbsp;<a href="?action=">菜单列表</a>&nbsp;&nbsp;&nbsp;&nbsp;<img src="../images/add.gif" width="14" height="14" />&nbsp;<a href="?action=add">添加菜单</a></p>
<?php
switch($_GET['action']){
    case 'order':
...
<?php
    ...
    <tr>
      ...
      <td width="200" height="25" align="center" background="../images/tab_14.gif">操作</td>
      </tr>
      <?php nclass_arr(0,0);?>
</table>
<?php
...
function delclass($classid){
    ...
    if($classid){
		...
		$db->query("delete from g_nclass_menu where locate(',$classid,',classpath)>0");
		return true;
	}else{
		return false;
	}
...
function savepath($newid,$parentid){
	...
	if($parentid==0){ 
		...
		$sql='select classpath from g_nclass_menu where classid='.(int)$parentid;
		$query = $db -> query($sql);
		if($row = $db -> fetch_array($query)){
			$classpath=$row['classpath'].$newid.','; //父节点的路径	
		}
	}
	return $db -> query("update g_nclass_menu set classpath='$classpath' where classid=".(int)$newid);
}
?>